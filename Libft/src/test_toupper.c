...
/*   Updated: 2025/02/05 22:24:15 by ide-dieg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void test_toupper(int d, int i, int e)
{
	char	*original;
	char	*libft;
	int		arg;
	...
	while (arg < 256)
	{
		...
		if (strcmp(original, libft) != 0)
		{
			if (strcmp(libft, "segfault") == 0)
			{
				...
				{
					if (isprint(arg))
						printf("char: (%c)%s[ko]%s[toupper: %s][ft_toupper: %s]\n", arg, COLOR_RED, COLOR_RESET, original, libft);
					else
						printf("char: (%d)%s[ko]%s[toupper: %s][ft_toupper: %s]\n", arg, COLOR_RED, COLOR_RESET, original, libft);
				}
				ko++;
			}
			else if ((atoi(original) && atoi(libft)) || (!atoi(original) && !atoi(libft)))
			{
				...
				{
					if (isprint(arg))
						printf("char: (%c)%s[ok]%s[toupper: %s][ft_toupper: %s]\n", arg, COLOR_YELLOW, COLOR_RESET, original, libft);
					else
						printf("char: (%d)%s[ok]%s[toupper: %s][ft_toupper: %s]\n", arg, COLOR_YELLOW, COLOR_RESET, original, libft);
				}
				iregular_ok++;
			}
			else
