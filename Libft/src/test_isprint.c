...
/*   Updated: 2025/02/05 03:39:52 by ide-dieg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void test_isprint(int d, int i, int e)
{
	char	*original;
	char	*libft;
	int		arg;
	...
	while (arg < 256)
	{
		exec_function(&original, arg, &isprint);
		exec_function(&libft, arg, &ft_isprint);
		if (strcmp(original, libft) != 0)
		{
			if (strcmp(libft, "segfault") == 0)
			{
				...
				{
					if (isprint(arg))
						printf("char: (%c)%s[ko]%s[isprint: %s][ft_isprint: %s]\n", arg, COLOR_RED, COLOR_RESET, original, libft);
					else
						printf("char: (%d)%s[ko]%s[isprint: %s][ft_isprint: %s]\n", arg, COLOR_RED, COLOR_RESET, original, libft);
				}
				ko++;
			}
			else if ((atoi(original) && atoi(libft)) || (!atoi(original) && !atoi(libft)))
			{
				...
				{
					if (isprint(arg))
						printf("char: (%c)%s[ok]%s[isprint: %s][ft_isprint: %s]\n", arg, COLOR_YELLOW, COLOR_RESET, original, libft);
					else
						printf("char: (%d)%s[ok]%s[isprint: %s][ft_isprint: %s]\n", arg, COLOR_YELLOW, COLOR_RESET, original, libft);
				}
				iregular_ok++;
			}
			else
